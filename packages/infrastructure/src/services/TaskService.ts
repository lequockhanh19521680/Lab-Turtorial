...
export class TaskService {
   ...
   */
  async updateTask(projectId: string, taskId: string, updates: UpdateTaskRequest): Promise<Task> {
    ...
    if ((updates.status === "DONE" || updates.status === "FAILED") && !updates.completedAt) {
   ...
   */
  private async areDependenciesSatisfied(task: Task, allTasks: Task[]): Promise<boolean> {
   ...
   */
  async rejectTask(projectId: string, taskId: string, reason?: string): Promise<Task> {
    return await this.updateTask(projectId, taskId, {
      status: "TODO",
      progress: 0,
      startedAt: undefined,
   ...
   */
  private validateStatusTransition(currentStatus: Task['status'], newStatus: Task['status']): void {
