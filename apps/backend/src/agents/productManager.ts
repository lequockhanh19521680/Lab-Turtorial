...
import { generateRequirements } from "../utils/ai.js";
// Update the import path if your models file is actually named 'models.ts'
...
export const extractFeatures = (requirements: any): string[] => {
  ...
  return requirements.features.filter(
    (feature: any) => typeof feature === "string" && feature.trim().length > 0
  );
};
...
export const generateUserStories = (requirements: any): string[] => {
  ...
  return requirements.userStories.filter(
    (story: any) => typeof story === "string" && story.trim().length > 0
  );
};
