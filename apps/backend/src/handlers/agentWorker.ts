...
const PROJECT_NOTIFICATION_TOPIC_ARN =
  process.env.PROJECT_NOTIFICATION_TOPIC_ARN!;
const FUNCTION_PREFIX =
  process.env.AWS_LAMBDA_FUNCTION_NAME?.split("-")[0] || "agent-builder";
...
export const handler = async (
  ...
  console.log(
    "Agent Worker processing SQS event:",
    ...
    try {
      ...
      if (response.StatusCode === 200 && response.Payload) {
        ...
        if (result.success) {
          ...
          console.log(
            `Agent ${agentName} completed and awaiting approval for project ${projectId}`
          );
