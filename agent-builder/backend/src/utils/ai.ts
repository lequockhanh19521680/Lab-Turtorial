 ...
 */
export const generateRequirements = async (prompt: string): Promise<{
  features: string[]
  userStories: string[]
 ...
 */
export const generateBackendSpecs = async (requirements: any): Promise<{
  apis: any[]
  database: any
  architecture: string
}> => {
 ...
 */
export const generateFrontendSpecs = async (requirements: any): Promise<{
  components: any[]
  pages: any[]
  routing: any[]
  styling: string
 ...
 */
export const generateDevOpsSpecs = async (backendSpecs: any, frontendSpecs: any): Promise<{
  infrastructure: any
  deployment: any
  monitoring: any
  security: any
...
- security: Security configurations and best practices`
  ...
  try {
    const response = await client.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: systemPrompt },
        { role: 'user', content: JSON.stringify({ backend: backendSpecs, frontend: frontendSpecs }) }
      ],
      temperature: 0.7,
      max_tokens: 2000
    })
...
function generateMockRequirements(_prompt: string) {
  return {
    ...
    architecture: 'Modern serverless architecture with React frontend and AWS Lambda backend'
  }
}
...
function generateMockBackendSpecs() {
  return {
    apis: [
      { method: 'GET', path: '/api/data', description: 'Get data', requestBody: null, responseBody: 'Array of data objects' },
      { method: 'POST', path: '/api/data', description: 'Create data', requestBody: 'Data object', responseBody: 'Created data object' }
    ],
    database: {
...
function generateMockFrontendSpecs() {
  return {
    components: [
      { name: 'Header', description: 'App header with navigation', props: ['title'], children: [] },
      { name: 'DataList', description: 'List of data items', props: ['data'], children: ['DataItem'] }
    ],
    pages: [
      { name: 'Home', route: '/', description: 'Homepage', components: ['Header', 'DataList'] },
      { name: 'Login', route: '/login', description: 'Login page', components: ['LoginForm'] }
    ],
    routing: [
      { path: '/', component: 'Home' },
