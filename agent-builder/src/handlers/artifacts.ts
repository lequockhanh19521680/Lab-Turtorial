...
import { ArtifactService, ProjectService } from "../services";
import {
...
export class ArtifactHandler {
      ...
      switch (method) {
        ...
        case "PATCH":
          ...
          if (!projectId || !artifactId) {
            return createErrorResponse(400, "Project ID and Artifact ID are required");
          }
          return await this.updateArtifact(projectId, artifactId, event);

        case "DELETE":
          const { projectId: delProjectId, artifactId: delArtifactId } = pathParameters;
   ...
   */
  private async getProjectArtifacts(projectId: string): Promise<APIGatewayProxyResult> {
    try {
      ...
      const artifacts = await this.artifactService.getProjectArtifacts(projectId);

      // Group artifacts by type for easier frontend consumption
      const artifactsByType = {
        SRS_DOCUMENT: artifacts.filter(a => a.artifactType === 'SRS_DOCUMENT'),
        SOURCE_CODE: artifacts.filter(a => a.artifactType === 'SOURCE_CODE'),
        DEPLOYMENT_URL: artifacts.filter(a => a.artifactType === 'DEPLOYMENT_URL'),
   ...
   */
  private async createArtifact(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    try {
  ...
  private async updateArtifact(
    ...
    try {
      ...
      const updatedArtifact = await this.artifactService.updateArtifact(projectId, artifactId, updates);
   ...
   * Delete an artifact
   */
  private async deleteArtifact(projectId: string, artifactId: string): Promise<APIGatewayProxyResult> {
    try {
