...
import { Artifact, CreateArtifactRequest } from "../models";
...
export class ArtifactService {
   ...
   */
  async getArtifactsByType(projectId: string, artifactType: Artifact['artifactType']): Promise<Artifact[]> {
   ...
   * Update artifact
   */
  async updateArtifact(projectId: string, artifactId: string, updates: Partial<Artifact>): Promise<Artifact> {
    // Validate artifact exists
    const existingArtifact = await this.artifactRepository.findById(projectId, artifactId);
    if (!existingArtifact) {
      throw new Error(`Artifact with ID ${artifactId} not found in project ${projectId}`);
    }
   ...
   */
  async getLatestArtifactByType(projectId: string, artifactType: Artifact['artifactType']): Promise<Artifact | null> {
    const artifacts = await this.artifactRepository.findByType(projectId, artifactType);
   ...
   */
  async getArtifactWithMetadata(projectId: string, artifactId: string): Promise<Artifact & { enrichedMetadata?: any } | null> {
    const artifact = await this.artifactRepository.findById(projectId, artifactId);
  ...
  private validateArtifactRequest(request: CreateArtifactRequest): void {
    ...
    switch (request.artifactType) {
      case 'SOURCE_CODE':
        if (!request.location.includes('github.com') && !request.location.includes('gitlab.com')) {
          // Allow local paths for development
          if (!request.location.startsWith('/') && !request.location.startsWith('file://')) {
            console.warn('Source code artifacts should typically be hosted on Git platforms');
          }
        }
        break;
      case 'DEPLOYMENT_URL':
  ...
  private async enrichArtifactMetadata(artifact: Artifact): Promise<any> {
    ...
    switch (artifact.artifactType) {
      case 'SOURCE_CODE':
        ...
        break;
      case 'DEPLOYMENT_URL':
        enriched.isAccessible = await this.checkUrlAccessibility(artifact.location);
        break;
    }
  ...
  private detectLanguageFromLocation(location: string): string {
    if (location.includes('typescript') || location.includes('.ts')) return 'TypeScript';
    if (location.includes('javascript') || location.includes('.js')) return 'JavaScript';
    if (location.includes('python') || location.includes('.py')) return 'Python';
